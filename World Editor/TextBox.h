...
public:
		...
		if (m_is_picked) {
			...
			if (event.type == sf::Event::TextEntered) {
				if (event.text.unicode < 128) {
					...
					else if (m_text.getString().toAnsiString().length() < m_max_value) { // Если вводим текст и строка короче заданного максимумв
	...
	// Задать данные в текстбокс
	void SetData(string data) {
		m_text.setString(data);
	}

	// Получить данные с текстбокса
	string GetData() {
